...
AMI_DIR = "D:/CODING/octernship/action-item-detection/AMI Dataset/MANUAL DATA"
DATASET_DIR = "D:/CODING/octernship/action-item-detection/AMI Dataset/MANUAL DATA"
...
SIMILARITY_PATH = "D:/CODING/octernship/action-item-detection/AMI Dataset/AMI/similarity/similarity_ngram_cosine_ami101a.json"
...
def get_default_meet2speakers():
    ...
    meet2speakers = {meet: sorted(speakers)
                     for meet, speakers in meet2speakers.items()}
...
def parse_extractive(link_path):
    ...
    # key: dialog_act_id, value: abstract_id list
...
def parse_dialogue_act(dialogue_act_path):
    ...
    for da_info in da_list:
        ...
        word_id = [int(re.findall(r"id\(.*words(\d*)\)", x)[0])
                   for x in word_id]
    ...
    da_dict = dict(sorted(da_dict.items(), key=lambda x: (
        x[1]["start_id"], x[1]["end_id"])))
...
def parse_words(words_path):
    ...
    for input_word_type in word_type_list:
        ...
        for input_word_node in word_node_list:
            input_word_data = get_word_data(
                input_word_node, word_type=input_word_type)
            if input_word_data:
...
def test_dialogue_act_link_words(meet="ES2013b", speaker="D"):
    ...
    dialogue_act_file = os.path.join(
        DACT_DIR, f"{meet}.{speaker}.dialog-act.xml")
...
def test_dialogue_act_link_label(meet="ES2013b", speaker="D"):
    ...
    abstractive_file = os.path.join(ABST_DIR, f"{meet}.abssumm.xml")
    dact_dict = dialogue_act_link_label(
        dialogue_act_file, extractive_file, abstractive_file)
    print(dact_dict)
    return dact_dict
...
def is_valid_meeting(meeting_name, remove_zero=True):
    ...
    candidate_ids = [
        idx for idx in abstract_action_ids if idx in extract_abstract_ids]
...
def default_sort_ami_data_list(ami_data_list):
    ...
    meet_types = {f"AMI#{meet_type}": idx for idx,
                  meet_type in enumerate(meet_types)}
    ami_data_list = sorted(ami_data_list, key=lambda x: (
        meet_types[x["data_source"]], x["meeting_name"], x["sentence_id"]))
...
def default_sort_meet_data_list(meet_data_list):
    meet_data_list = sorted(meet_data_list, key=lambda x: (
...
def status_ami_data_list(ami_data_list, detail_type=False, detail_file=False, detail_document=False):
    ...
    print(
        f"## Meeting: {count_meeting} (IS: {count_is}, ES: {count_es}, TS: {count_ts})")
    print(f"## All Sentence: {count_sentence}, Positive: {count_positive} ({rate_positive:.2%}), "
          f"Negative: {count_negative} ({rate_negative:.2%})\n")

    if detail_document:
        count_sentence_list = [status_item["num_sentence"]
                               for meeting_name, status_item in meeting_dict.items()]
        count_positive_list = [status_item["num_positive"]
                               for meeting_name, status_item in meeting_dict.items()]
        count_negative_list = [status_item["num_negative"]
                               for meeting_name, status_item in meeting_dict.items()]
...
def scenario_only_dataset_split(ami_data_list):
    ...
    train_data_list = [
        item for item in ami_data_list if item["dataset"] == "train"]
    print("\nTraining Dataset:")
    status_ami_data_list(train_data_list, detail_document=True)

    dev_data_list = [
        item for item in ami_data_list if item["dataset"] == "dev"]
    print("\nDevelopment Dataset:")
    status_ami_data_list(dev_data_list, detail_document=True)

    test_data_list = [
        item for item in ami_data_list if item["dataset"] == "test"]
    print("\nTest Dataset:")
...
def single_select_ami_data_list(data_list, select_key, select_values):
    result_list = [
        item for item in data_list if item[select_key] in select_values]
    return result_list
...
def select_ami_data_list(data_list, select_dict):
    ...
    for select_key, select_values in select_dict.items():
        result_list = single_select_ami_data_list(
            result_list, select_key, select_values)
    return result_list
...
def add_new_field(result_data_list, ami_data_list, new_field_list=None,
    ...
    # key: meeting_name, value: key: sentence_id, value: sentence
    ...
    for data_item in tqdm(result_data_list, desc="Add Fields: "):
        ...
        if new_field in new_field_list:
            global_context_list = []
            similarity_list = similarity_map[current_meeting_name].get(
                str(current_sentence_id), None)
            if similarity_list is not None:
                for global_idx in range(num_global_context):
                    ...
                    global_item = dataset_dict[current_meeting_name].get(
            ...
            global_context = context_sep.join(global_context_list) if len(
                global_context_list) > 0 else context_sep
        ...
        if new_field in new_field_list:
            ...
            for left_sentence_id in range(current_sentence_id - 1, current_sentence_id - num_left_context - 1, -1):
                if current_meeting_name not in dataset_dict:
                    continue
                left_item = dataset_dict[current_meeting_name].get(
                    left_sentence_id, None)
            ...
            left_context = context_sep.join(left_context_list) if len(
                left_context_list) > 0 else context_sep
        ...
        if new_field in new_field_list:
            ...
            for right_sentence_id in range(current_sentence_id + 1, current_sentence_id + num_right_context + 1):
                if current_meeting_name not in dataset_dict:
                    continue
                right_item = dataset_dict[current_meeting_name].get(
            ...
            right_context = context_sep.join(right_context_list) if len(
                right_context_list) > 0 else context_sep
...
def balance_data_list(data_list):
    ...
    balance_times = int(len(max_data_list) / len(min_data_list)
                        ) if len(min_data_list) != 0 else 0
    new_data_list = []
    last_j = 0
...
def write_document(meet_lines, file_path, field_list=None, default_value="###"):
    ...
    with open(file_path, "w", encoding="utf-8", newline='') as fw:
        ...
        writer.writerow(field_list)  # Write the header row
...
def output_ami_document(data_list, condition, file_path, field_list=None,
    ...
    new_field_list = [
        field for field in field_list if field not in meetline_fields]
...
def output_ami_dataset(data_list, dataset_path, supply_fields=None,
    ...
    train_file = os.path.join(dataset_path, train_file_name)
    condition = {"dataset": ["train"]}
    output_ami_document(data_list, condition, train_file, field_list=supply_fields,
    ...
    validation_file = os.path.join(dataset_path, validation_file_name)
    condition = {"dataset": ["validation"]}
    output_ami_document(data_list, condition, validation_file, field_list=supply_fields,
                        num_context_left=num_context_left, num_context_right=num_context_right,
                        num_global_context=num_context_global, add_context_label=add_context_label,
    ...
    test_file = os.path.join(dataset_path, test_file_name)
    condition = {"dataset": ["test"]}
    output_ami_document(data_list, condition, test_file, field_list=supply_fields,
                        num_context_left=num_context_left, num_context_right=num_context_right,
                        num_global_context=num_context_global, add_context_label=add_context_label,
                        context_sep=context_sep, context_label_sep=context_label_sep, seed=seed)
...
def ami_process_dialogue_act():
    ...
    for meet, speakers in tqdm(meet2speakers.items()):
        ...
        for speaker in speakers:
            words_file = os.path.join(WORD_DIR, f"{meet}.{speaker}.words.xml")
            dialog_act_file = os.path.join(
                DACT_DIR, f"{meet}.{speaker}.dialog-act.xml")
            if not os.path.exists(words_file) or not os.path.exists(dialog_act_file):
                continue
            if not is_valid_meeting(meet, remove_zero=True):
                continue
            speaker_dact_dict = dialogue_act_link_words(
                dialog_act_file, words_file)
            meet_dact_dict.update(speaker_dact_dict)

        meet_dact_dict = dialogue_act_link_label(
            meet_dact_dict, extract_file, abstract_file)
    ...
    status_ami_data_list(ami_data_list, detail_type=True,
                         detail_file=True, detail_document=True)
